...
?>
			<table class="form-table" style="margin-bottom: 15px;">
				<tr>
					...
					<td class="dsidx-<?php echo $diagnostics["IsAccountValid"] ? "success" : "failure" ?>">
						<?php echo $diagnostics["IsAccountValid"] ? "Yes" : "No" ?>
					</td>
					
					<th style="width: 230px;">API key active?</th>
					<td class="dsidx-<?php echo $diagnostics["IsApiKeyValid"] ? "success" : "failure" ?>">
						<?php echo $diagnostics["IsApiKeyValid"] ? "Yes" : "No" ?>
					</td>
				</tr>
				<tr>
					<th>Account authorized for this MLS?</th>
					<td class="dsidx-<?php echo $diagnostics["IsAccountAuthorizedToMLS"] ? "success" : "failure" ?>">
						<?php echo $diagnostics["IsAccountAuthorizedToMLS"] ? "Yes" : "No" ?>
					</td>
					
					<th>API key authorized for this blog?</th>
					<td class="dsidx-<?php echo $diagnostics["IsApiKeyAuthorizedToUri"] ? "success" : "failure" ?>">
						<?php echo $diagnostics["IsApiKeyAuthorizedToUri"] ? "Yes" : "No" ?>
					</td>
				</tr>
				<tr>
					<th>Clock accurate on this server?</th>
					<td class="dsidx-<?php echo $diagnostics["ClockIsAccurate"] ? "success" : "failure" ?>">
						<?php echo $diagnostics["ClockIsAccurate"] ? "Yes" : "No" ?>
					</td>
					
					<th>API key authorized for this server?</th>
					<td class="dsidx-<?php echo $diagnostics["IsApiKeyAuthorizedToIP"] ? "success" : "failure" ?>">
						<?php echo $diagnostics["IsApiKeyAuthorizedToIP"] ? "Yes" : "No" ?>
					</td>
				</tr>
				<tr>
					<th>WordPress link structure ok?</th>
					<td class="dsidx-<?php echo $diagnostics["UrlInterceptSet"] ? "success" : "failure" ?>">
						<?php echo $diagnostics["UrlInterceptSet"] ? "Yes" : "No" ?>
					</td>
					
					<th>Under monthly API call limit?</th>
					<td class="dsidx-<?php echo $diagnostics["UnderMonthlyCallLimit"] ? "success" : "failure" ?>">
						<?php echo $diagnostics["UnderMonthlyCallLimit"] ? "Yes" : "No" ?>
					</td>
				</tr>
				<tr>
					<th>Server PHP version at least 5.2?</th>
					<td class="dsidx-<?php echo $diagnostics["PhpVersionAcceptable"] ? "success" : "failure" ?>">
						<?php echo $diagnostics["PhpVersionAcceptable"] ? "Yes" : "No" ?>
					</td>
					
					<th>Would you like fries with that?</th>
					<td class="dsidx-success">
						Yes <!-- you kidding? we ALWAYS want fries. mmmm, friessssss -->
					</td>
				</tr>
				<tr>
					<th colspan="4" style="text-align: center;">
						** Current status: <?php echo $diagnostics["DiagnosticsSuccessful"] ? "ACTIVE" : "INACTIVE" ?> **
					</th>
				</tr>
			</table>
<?php
...
<?php
	...
	static function RunDiagnostics($options) {
		...
		$diagnostics["IsApiKeyValid"] = $diagnostics["IsApiKeyValid"];
		$diagnostics["IsAccountAuthorizedToMLS"] = $diagnostics["IsAccountAuthorizedToMLS"];
		$diagnostics["IsAccountValid"] = $diagnostics["IsAccountValid"];
		$diagnostics["IsApiKeyAuthorizedToUri"] = $diagnostics["IsApiKeyAuthorizedToUri"];
		$diagnostics["IsApiKeyAuthorizedToIP"] = $diagnostics["IsApiKeyAuthorizedToIP"];
		
		$diagnostics["PhpVersionAcceptable"] = PHP_VERSION_ID > 50200;
		$diagnostics["UrlInterceptSet"] = get_option("permalink_structure") != "";
		$diagnostics["ClockIsAccurate"] = $timeDiff < $secondsIn2Hrs && $timeDiff > -1 * $secondsIn2Hrs;
		$diagnostics["UnderMonthlyCallLimit"] = $diagnostics["AllowedApiRequestCount"] > $diagnostics["CurrentApiRequestCount"];
		
		$diagnostics["DiagnosticsSuccessful"] = true;
		foreach ($diagnostics as $key => $value) {
			if (!$value)
				$diagnostics["DiagnosticsSuccessful"] = false;
		}
